...
exports.processUrl = async (url) => {
  if (isPlaylistUrl(url)) {
    ...
    const playlistItemsArgs = [url, '--flat-playlist', '--dump-json'];

    if (config.playlistDownloadLimit) {
      playlistItemsArgs.push('--playlist-end', String(config.playlistDownloadLimit));
    }

    try {
      const stdout = await ytdlp.execPromise(playlistItemsArgs);
      const videoInfos = stdout
        .trim()
        .split('\n')
        .filter((line) => line)
      ...
      if (
        config.maxQueueLimit &&
        activeDownloads + downloadQueue.length + videoInfos.length > config.maxQueueLimit
      ) {
      ...
      return {
        status: 202,
        message: `Playlist processed. Added ${videoInfos.length} videos to the queue`
      };
    } catch (error) {
      ...
      throw new CustomError(
        'Failed to fetch playlist details. The playlist may be empty or invalid',
        500
      );
    }
  }
...
exports.addToQueue = (url) => {
  if (
    config.maxQueueLimit &&
    activeDownloads + downloadQueue.length >= config.maxQueueLimit
  ) {
    console.warn(
      `[Queue] Total job limit reached. Rejecting new download request. Limit: ${config.maxQueueLimit}`
    );
    throw new CustomError(
      `Queue limit reached (max: ${config.maxQueueLimit}). Please try again later`,
      429
    );
  }
  ...
  if (activeDownloads > 0) {
    ...
    console.log(
      `[Queue] Download added to queue. Current queue size: ${downloadQueue.length}`
    );
...
async function processNextInQueue() {
  ...
  console.log(
    `[Queue] Starting next download from queue. Queue size: ${downloadQueue.length}`
  );
...
function executeDownload(url, videoData, commonArgs) {
  return new Promise((resolve, reject) => {
    ...
    const downloadArgs = [
      url,
      '-f',
      'bestvideo[vcodec^=avc1]+bestaudio[acodec^=mp4a]/best[vcodec^=avc1]/best[ext=mp4]/best',
      '-o',
      path.join(config.videosPath, filename),
      '--progress',
      '--no-part',
      '--merge-output-format',
      'mp4',
...
function handleDownloadError(error, videoId) {
  let userMessage =
    'An unknown download error occurred. Please check the server logs for more details';
  ...
  if (errorMessage.includes('unsupported url')) {
    ...
    userMessage = 'Cookie file permission denied. Please restart MEDIS';
  } else if (errorMessage.includes('sign in') || errorMessage.includes('403: forbidden')) {
    userMessage =
      'Download failed: This video may be private or regionally restricted. Using a cookies file might help';
  } else if (errorMessage.includes('the downloaded file is empty')) {
    userMessage =
      'Download failed: The file is empty, possibly due to regional restrictions or protection';
  } else if (errorMessage.includes('404')) {
    userMessage = 'Could not find the video. Please check the URL';
  } else if (errorMessage.includes('metadata extraction failed')) {
    userMessage =
      'Failed to get video info. The URL might be incorrect or private';
  }
  ...
  cleanupIncompleteFiles(videoId).catch((cleanupErr) => {
    console.error(
      `[Cleanup] Nested error during cleanup for ${videoId}`,
      cleanupErr
    );
  });
}
...
async function cleanupIncompleteFiles(baseFilename) {
  try {
    ...
    console.error(
      `[Cleanup] Error during file cleanup for ${baseFilename}`,
      err
    );
  }
}
